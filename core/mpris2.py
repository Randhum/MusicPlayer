...
class MPRIS2Service(dbus.service.Object):
    ...
    def Get(self, interface_name: str, property_name: str):
        ...
        try:
            ...
            logger.error("MPRIS2: Error getting property %s.%s: %s", interface_name, property_name, e, exc_info=True)
            raise dbus.exceptions.DBusException(
                f"{self.__class__.__name__}.Error",
                f"Error getting property: {e}"
            )
    ...
    def Metadata(self, value: Dict[str, Any]):
        ...
        if self._metadata != value:
            ...
            # Since metadata is already a{sv}, we wrap the whole dict in a Variant
            try:
                from dbus import types
                # Convert metadata dict to proper D-Bus format
                # Each value in the metadata dict should be a Variant
                dbus_metadata = {}
                for key, val in value.items():
                    if isinstance(val, dbus.ObjectPath):
                        ...
                        dbus_metadata[key] = dbus.Array([dbus.String(str(v)) for v in val], signature='s')
                    elif isinstance(val, (int, float)):
                        # Numbers - D-Bus will convert automatically, but be explicit for Int64
                        if key == 'mpris:length':
                            dbus_metadata[key] = dbus.Int64(val)
                        elif key == 'xesam:trackNumber':
                            dbus_metadata[key] = dbus.Int32(val)
                        else:
                            dbus_metadata[key] = val
                    else:
                        # Strings and other types - ensure they're strings
                        dbus_metadata[key] = dbus.String(str(val))
                
                # Wrap the metadata dict in a Variant for PropertiesChanged
                changed_props = {'Metadata': types.Variant('a{sv}', dbus_metadata)}
                self.PropertiesChanged(MPRIS2_PLAYER_INTERFACE, changed_props, [])
            except Exception as e:
                logger.error("MPRIS2: Error emitting PropertiesChanged for Metadata: %s", e, exc_info=True)
                # Fallback: try with native types (let D-Bus convert automatically)
                try:
                    self.PropertiesChanged(MPRIS2_PLAYER_INTERFACE, {'Metadata': value}, [])
                except Exception as e2:
                    logger.error("MPRIS2: Fallback PropertiesChanged also failed: %s", e2, exc_info=True)
